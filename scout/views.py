...
from scout.dao.image import get_item_image, get_spot_image
from scout.dao.item import get_filtered_items, get_item_by_id, get_item_count
...
def validate_campus_selection(function):
    def wrap(request, *args, **kwargs):
        if settings.CAMPUS_URL_LIST and isinstance(settings.CAMPUS_URL_LIST, list):
...
class DiscoverCardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        discover_categories = {
            "safestudy": {
                ...
                "filter": [
                    ("limit", 5),
                    ("open_now", True),
                    ("center_latitude", hlat if hlat else lat if lat else DEFAULT_LAT),
                    ("center_longitude", hlon if hlon else lon if lon else DEFAULT_LON),
                    ("distance", 100000),
                ],
            },
            "safefood": {
        ...
        discover_data["filter"].append(("extended_info:campus", kwargs["campus"]))
