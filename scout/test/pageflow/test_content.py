...
class ContentTest(ScoutTestCase):
    ...
    def test_404_static_navigation(self):
        ...
        self.assertHTMLEqual(
            str(study.find(id="link_privacy")), str(soup404.find(id="link_privacy"))
        )
        """Test Terms of Use"""
        self.assertHTMLEqual(
            str(study.find(id="link_terms")), str(soup404.find(id="link_terms"))
        )
        """Test UW Help"""
        self.assertHTMLEqual(
    ...
    def test_404_context_navigation(self):
        ...
        """Test home buttons"""
        self.assertHTMLEqual(
            str(study_seattle.find(id="link_home")["href"]),
            str(soup404_seattle.find(id="link_home")["href"]),
        )
        self.assertHTMLEqual(
            str(study_tacoma.find(id="link_home")["href"]),
            str(soup404_tacoma.find(id="link_home")["href"]),
        )
        self.assertHTMLEqual(
            str(study_bothell.find(id="link_home")["href"]),
            str(soup404_bothell.find(id="link_home")["href"]),
        )

        """Test Food buttons"""
        self.assertHTMLEqual(
            str(study_seattle.find(id="link_food")["href"]),
            str(soup404_seattle.find(id="link_food")["href"]),
        )
        self.assertHTMLEqual(
            str(study_tacoma.find(id="link_food")["href"]),
            str(soup404_tacoma.find(id="link_food")["href"]),
        )
        self.assertHTMLEqual(
            str(study_bothell.find(id="link_food")["href"]),
            str(soup404_bothell.find(id="link_food")["href"]),
        )

        """Test study buttons"""
        self.assertHTMLEqual(
            str(study_seattle.find(id="link_study")["href"]),
            str(soup404_seattle.find(id="link_study")["href"]),
        )
        self.assertHTMLEqual(
            str(study_tacoma.find(id="link_study")["href"]),
            str(soup404_tacoma.find(id="link_study")["href"]),
        )
        self.assertHTMLEqual(
            str(study_bothell.find(id="link_study")["href"]),
            str(soup404_bothell.find(id="link_study")["href"]),
        )

        """Test tech buttons"""
        self.assertHTMLEqual(
            str(study_seattle.find(id="link_tech")["href"]),
            str(soup404_seattle.find(id="link_tech")["href"]),
        )
        self.assertHTMLEqual(
            str(study_tacoma.find(id="link_tech")["href"]),
            str(soup404_tacoma.find(id="link_tech")["href"]),
        )
        self.assertHTMLEqual(
            str(study_bothell.find(id="link_tech")["href"]),
